  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center gap-10 px-6 py-16">
      <div className="flex flex-col gap-4">
        ...
        <h1 className="page-title max-w-xl">The board Gilfoyle writes before a cloud agent starts.</h1>
        <p className="max-w-xl text-muted-foreground">
          Richard hears a Slack mention, a GitHub mention of avinitDollarpe or @cursor, or you delegate in chat. Gilfoyle opens the task here. An active Cursor plan launches the agent. Anything else stays in blocked:cursor_plan.
        </p>
      </div>
      <ol className="flex max-w-xl flex-col gap-2 text-sm text-muted-foreground">
        <li>Chief receives the trigger and hands Gilfoyle the context.</li>
        <li>Fleetglass records the task once, even if the webhook repeats.</li>
        <li>The plan gate launches Cursor, or blocks the task.</li>
      </ol>
      <form action={requestMagicLink} className="card flex flex-col gap-3 p-4 sm:max-w-md">
        ...
        {query.error === "email" ? <p className="text-sm text-danger">Enter a valid email.</p> : null}
