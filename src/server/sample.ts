...
export async function loadSampleFleet(userId: string) {
  ...
  const parent = await createTask(userId, {
    ...
    payload: {
      ...
      commentBody: "@fleetglass the retry budget still drops the second charge",
  ...
  const follow = await createTask(userId, {
    ...
    payload: {
      repo: "example/checkout",
      prNumber: 148,
      commentBody: "@fleetglass also cover partial capture",
      commenter: "chakravarti",
      mentionTargets: ["fleetglass"],
  ...
  const slack = await createTask(userId, {
    ...
    payload: {
      ...
      text: "@Fleetglass watch the checkout agent through Watching 2/3",
      user: "chakravarti",
