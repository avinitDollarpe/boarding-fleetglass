...
`reply` is present only on a Slack brief, and only when `FLEETGLASS_PUBLIC_URL` and a reply secret both exist. The base is that URL with no trailing slash. `VERCEL_URL` is not a reply base. The secret is `FLEETGLASS_REPLY_SECRET` when set, otherwise `SLACK_SIGNING_SECRET`. `exp` is a Unix second, 15 minutes ahead. `sig` is HMAC-SHA256 hex over `JSON.stringify(["v1", channel_id, thread_ts, exp])`. The text of the later reply is not inside the HMAC. GitHub briefs omit `reply`. Fleetglass omits `reply` when the public URL is unset, not http(s), localhost, or a `*-git-*.vercel.app` host. The wake still runs.
